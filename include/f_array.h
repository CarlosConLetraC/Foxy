#ifndef FOXY_ARRAY_H
    #define FOXY_ARRAY_H

    #include <stddef.h>
    #include <stdbool.h>
    #include "f_value.h" // ya hay una macro en f_value.h para evitar dependencias circulares.

    typedef struct FoxyArray {
        FoxyValue *items;
        size_t capacity;
        size_t count;
        FoxyValueType array_type; // FOXY_VAL_NULL o FOXY_VAL_ANY para heterogéneos
    } FoxyArray;

    FoxyArray* f_array_new_typed(size_t initial_capacity, FoxyValueType array_type);

    // Interfaz pública
    FoxyArray* f_array_new(size_t initial_capacity);
    void f_array_free(FoxyArray *array);
    bool f_array_push(FoxyArray *array, FoxyValue value);
    bool f_array_pop(FoxyArray *array, FoxyValue *out_value);
#endif // FOXY_ARRAY_H